...
fail() {
  echo "docs-feature-write check failed: $*" >&2
  exit 1
}
...
grep -q '^name: docs-feature-write$' "$skill_file" || fail "skill name changed"
...
grep -q 'docs-feature-style' "$skill_file" || fail "style enforcer handoff is missing"
...
echo "docs-feature-write check passed"
