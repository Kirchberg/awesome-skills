...
grep -q '\.git/pr-review-monitor-state\.json' "$state_ref" || fail "state file location is missing"
grep -q 'sleep 600' "$skill_file" || fail "10-minute monitor cadence is missing"
grep -q 'Thread Automations' "$state_ref" || fail "thread automation section is missing"
...
grep -q 'Do not create a new agent session while waiting' "$skill_file" || fail "session persistence rule is missing"
grep -q 'Connector review monitoring complete' "$skill_file" || fail "completion summary is missing"
